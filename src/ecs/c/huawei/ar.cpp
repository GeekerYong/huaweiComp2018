...
int main(int argc,char* argv[]){
    ...
    ifstream infile;   //时序输入流
    ofstream outfile;   //系数输出流
    string path = "./";
    string input_fname = string(argv[1]);
    string output_fname = string(argv[2]);
    string inputfile = path + input_fname;
    string ouputfile = path + output_fname;
    infile.open(inputfile.data());
    outfile.open(ouputfile.data());

    cout<<"analysis time series data"<<endl;

    double* inputseries = new double[100];
    const int LINE_LENGTH = 100;
    char str[LINE_LENGTH];
    int cnt = 0;
    while( infile.getline(str,LINE_LENGTH) )
    {
        inputseries[cnt] = strtod(str, NULL);
    ...
    cout<<"build ar model"<<endl;
    AutoRegression(inputseries,len,degree,coefficients,method);
    cout<<"model --success"<<endl;

    for(int i =0; i<degree;i++){
        outfile<<coefficients[i]<<endl;
    }
}
